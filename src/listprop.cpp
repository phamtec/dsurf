...
float ListProp::layout(Resources &res, float x, float y) {
  ...
  return _r.h + (_objs.size() > 0 ? res.close_bracket.height() : 0); // closed bracket if not empty
}
...
void ListProp::render(Renderer &renderer, Resources &res) {
  ...
  res.open_bracket.render(renderer, _r.x + _name.width() + Sizes::text_padding, _r.y);
  List::renderVector(renderer, res, _objs);
  res.close_bracket.render(renderer, 
    _r.x + (_objs.size() == 0 ? _name.width() + res.open_bracket.width() + (Sizes::text_padding * 2): 0), 
